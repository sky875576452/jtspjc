  ...
  <div ref="user">
    ...
    <el-form
      ...
      <el-form-item label="通行方向">
        <el-select
          size="small"
          filterable
          v-model="formInline.direction" placeholder="请选择">
          <el-option v-for="(item,index) in optionData.DIRECTION" :key="index" :label="item.name" :value="item.code"></el-option>
        </el-select>
      </el-form-item>
      </el-col>
      <el-col :span="12">
    ...
    >
      ...
      <el-table-column
        ...
        width="160">
        <template slot-scope="scope">
          <el-button plain type="primary" v-if="scope.row.checkStatus=='0'" @click.native="handleClick(scope.row)" size="mini">核查</el-button>
          <el-button plain type="primary" v-else size="mini" @click="$router.push({path:'details',query:{id:scope.row.id}})">查看</el-button>
        </template>
      </el-table-column>
    </commonTable>
  </div>
