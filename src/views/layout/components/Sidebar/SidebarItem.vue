  ...
  <div class="menu-wrapper">
    <template v-for="item in routes" v-if="!item.hidden&&item.children">
      ...
      <router-link
        ...
        :to="{path:item.path+'/'+item.children[0].path,query:{src:item.children[0].meta.iframeUrl}}"
      ...
      <el-submenu v-else :index="item.name||item.path" :key="item.name">
        ...
        <template v-for="child in item.children" v-if="!child.hidden">
          ...
          <router-link v-else
            :to="{path:item.path+'/'+child.path,query:{src:child.meta.iframeUrl}}"
            :key="child.name">
    ...
    <!-- <template v-else>
      <router-link
        :to="{path:item.path,query:{src:item.meta.iframeUrl}}"
        :key="item.name">
